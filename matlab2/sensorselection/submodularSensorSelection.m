function [S] = submodularSensorSelection(A, varargin)
...
for i = 1:2:length(varargin)
    if strcmpi(varargin{i}, 'gramT')
        gramT = varargin{i + 1};
    elseif strcmpi(varargin{i}, 'maxSensors')
...
while numel(S) < maxSensors
    ...
    for i=1:numel(R)
        ...
        G = zeros(size(A));         % construct new gramian
        for t=0:gramT               % varry finite time
            G = G + ((At(string(t))') * (C') * C * At(string(t)));
        end
        if subCriteria == 1
