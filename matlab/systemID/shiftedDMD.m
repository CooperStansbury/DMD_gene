function [Output] = shiftedDMD(DataMatrix,replicates,MetaData,thresh)
...
n = size(DataMatrix,1);
% X = zeros(n, replicates * (t-1));
% Xp = zeros(n, replicates * (t-1));
X = [];
Xp = [];
for r=1:replicates
    ...
    X = [X R(:,1:end-1)];
    Xp = [Xp R(:,2:end)];
    % X(:, [t*(r-1)+1 : t*r-1]) = R(:,1:end-1);
    % Xp(:,[t*(r-1)+1 : t*r-1]) = R(:,2:end);
end
